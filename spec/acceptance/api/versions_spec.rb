...
RSpec.resource 'Versions' do
  ...
  get '/api/projects/:project_id/checklists/:checklist_id/versions/:id' do
    let(:project_id)   { project.id }
    let(:checklist_id) { checklist.id }
    let(:id)           { version.id }
    example_request 'Show Version' do
  ...
  post '/api/projects/:project_id/checklists/:checklist_id/versions/:id/share' do
    let(:project_id)   { project.id }
    let(:checklist_id) { checklist.id }
    let(:id)           { version.id }

    example_request 'Share Version' do
      expect(status).to eq(200)
    end
  end
